...
async function refreshAccessToken(): Promise<string> {
  ...
  try {
    const { data } = await refreshClient.post<AuthTokens>('/auth/refresh-token', { refreshToken });
    setSession(data);
  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry && !isAuthEndpoint && !skipsSessionRefresh) {
      ...
      try {
        ...
        // No window.location here: clearSession() above already flipped
        // isAuthenticated to false, and the root navigator (app/navigation)
        // reacts to that by switching back to the Auth stack on its own.
