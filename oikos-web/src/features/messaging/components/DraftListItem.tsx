...
export function DraftListItem({ draft, onDelete, isDeleting = false }: DraftListItemProps) {
  return (
    <li className="flex items-center gap-1 border-b border-gray-100 pr-2">
      <Link
        to={`/messages/new?draftId=${draft.id}`}
        className="flex min-w-0 flex-1 flex-col gap-0.5 px-3 py-2.5 hover:bg-gray-50"
      >
        <div className="flex items-baseline justify-between gap-2">
          <span className="truncate text-theme-sm font-medium text-gray-700">{draftTitle(draft)}</span>
      ...
      <button
        ...
        className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-error-500 disabled:cursor-not-allowed disabled:opacity-60"
      >
