...
export function UnitTypePriceRow({ propertyId, unitTypeId, label, currentPrice }: UnitTypePriceRowProps) {
  ...
  return (
    <form
      ...
      className="flex flex-col gap-2 border-b border-slate-200 py-3 last:border-b-0 sm:flex-row sm:items-end sm:justify-between"
      noValidate
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:gap-4">
        <p className="w-40 pb-2 text-sm font-medium text-slate-900 sm:pb-0">{label}</p>
        <Input
          label="Prix"
          type="number"
          min={0}
