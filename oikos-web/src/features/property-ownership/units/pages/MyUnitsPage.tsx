          ...
          <ul className="flex flex-col divide-y divide-gray-100 dark:divide-gray-800">
            {units.data.map((unit) => (
              <li key={unit.unitId} className="flex items-center justify-between py-2 text-sm">
                <Link to={`/property-ownership/units/${unit.propertyId}/${unit.unitId}`} className="text-gray-700 dark:text-gray-300 hover:underline">
                  {unit.propertyName} — {unit.buildingName} — Lot {unit.unitNumber}
                </Link>
                <span className="text-gray-500 dark:text-gray-400">{unit.ownershipShare}%</span>
              </li>
            ))}
          </ul>
        )}
