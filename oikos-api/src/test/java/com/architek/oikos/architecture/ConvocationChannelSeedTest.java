 ...
 * so can drift.
 *
 * <p>V6 creates the table and fills it, for prod and docker. The dev profile
 * cannot apply V6 - it runs on H2 with Flyway disabled and a schema generated
 * by Hibernate, so a migration dropping columns would fail on the first
 * statement - and loads V7 as a plain script through spring.sql.init instead.
 * Two files, one catalog: exactly the shape that made
 * {@link RolePermissionSeedTest} necessary for the RBAC bundles, one table
 * further along. A channel added to V6 alone works in prod and offers nothing
 * in dev.
 *
...
class ConvocationChannelSeedTest {
    ...
    private static final Path CATALOG_MIGRATION = RESOURCES.resolve("db/migration/V6__convocation_channels_and_reply_source.sql");
    private static final Path SEED_MIGRATION = RESOURCES.resolve("db/migration/V7__seed_convocation_channel.sql");
    ...
    void both_files_declare_the_same_catalog() throws IOException {
        assertThat(channelsIn(SEED_MIGRATION))
                .as("V7 re-seeds for dev what V6 seeds for prod - a channel in one and not the other is a "
    ...
    void the_dev_profile_loads_the_seed() throws IOException {
        assertThat(Files.readString(DEV_PROFILE))
                .as("application-dev.yml must list V7 in spring.sql.init.data-locations - without it the dev "
    ...
    void the_seed_is_replayable() throws IOException {
        // It is loaded on every dev start-up, and applied as a migration in prod where V6 has
     ...
     * quoted value of each tuple, whichever form the file uses: V6 lists them as
     * VALUES tuples, V7 as one guarded SELECT apiece.
     */
