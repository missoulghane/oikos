...
class MessageTest {
    ...
    @Test
    void posting_a_message_captures_sender_body_and_timestamp() {
        MessageId id = MessageId.newId();
        ConversationId conversationId = ConversationId.newId();
        EntityId senderId = EntityId.newId();
        Instant now = Instant.parse("2026-01-01T10:00:00Z");

        Message message = Message.post(id, conversationId, senderId, MessageBody.of("Bonjour"), now);
    ...
    void two_messages_with_the_same_id_are_equal() {
        MessageId id = MessageId.newId();
        Message first = Message.post(id, ConversationId.newId(), EntityId.newId(), MessageBody.of("a"), Instant.EPOCH);
        Message second = Message.post(id, ConversationId.newId(), EntityId.newId(), MessageBody.of("b"), Instant.EPOCH);
