...
public class GetUserService implements GetUserUseCase {
    ...
    public UserView getUser(GetUserQuery query) {
        return findUser(query).orElseThrow(() -> new UserNotFoundException(query.userId()));
    }

    @Override
    @Transactional(readOnly = true)
    public Optional<UserView> findUser(GetUserQuery query) {
        return userRepository.findById(query.userId()).map(UserView::of);
    }
}
