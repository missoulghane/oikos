 ...
 * (ProvisionInvitedAccountUseCase, GetUserUseCase, GrantPropertyRoleUseCase),
 * never to user's repository directly (rule 6). The only place invitation's
 * stored targetRole String is resolved back into user's own PropertyRole
 * enum, right before calling into user's port.
 */
@Component
public class InvitationAccountDirectoryAdapter implements AccountDirectoryPort {

    private final ProvisionInvitedAccountUseCase provisionInvitedAccountUseCase;
    private final GetUserUseCase getUserUseCase;
    private final GrantPropertyRoleUseCase grantPropertyRoleUseCase;

    public InvitationAccountDirectoryAdapter(ProvisionInvitedAccountUseCase provisionInvitedAccountUseCase,
                                              GetUserUseCase getUserUseCase, GrantPropertyRoleUseCase grantPropertyRoleUseCase) {
        this.provisionInvitedAccountUseCase = provisionInvitedAccountUseCase;
        this.getUserUseCase = getUserUseCase;
        this.grantPropertyRoleUseCase = grantPropertyRoleUseCase;
    }

    @Override
    public EntityId provisionAccount(EmailVO email, String fullName, RawPassword password) {
        UserId userId = provisionInvitedAccountUseCase.provision(new ProvisionInvitedAccountCommand(email, fullName, password));
        return EntityId.of(userId.asUuid());
    }
