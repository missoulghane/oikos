 ...
 * (GetUnitUseCase), never to property's repository directly (rule 4/6).
 * Named distinctly from installment's own InstallmentUnitDirectoryAdapter to
 * avoid a Spring bean name collision between same-named classes in different
 * packages.
 */
@Component
public class AccountingUnitDirectoryAdapter implements UnitDirectoryPort {
    ...
    public AccountingUnitDirectoryAdapter(GetUnitUseCase getUnitUseCase) {
