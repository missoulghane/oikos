...
/**
 * Cross-feature adapter: verifies a property exists (used by
 * CreateAccountService when creating a PROPERTY-type account) by delegating
 * to property's public port-in (GetPropertyUseCase), never to property's
 * repository directly (rule 4). Named distinctly from property's own
 * PropertyPartyDirectoryAdapter and installment's own
 * InstallmentPropertyDirectoryAdapter to avoid a Spring bean name collision
